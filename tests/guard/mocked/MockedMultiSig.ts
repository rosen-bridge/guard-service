...
when(
  mockedMultiSigHandlerInstance.sign(anything(), anything(), anything())
).thenThrow(Error('multiSig sign mocked to throw this error'));
when(
  mockedMultiSigHandlerInstance.sign(
    anything(),
    anything(),
    anything(),
    anything()
...
when(mockedMultiSigHandler.getInstance(anything(), anything())).thenReturn(
  instance(mockedMultiSigHandlerInstance)
);
...
export { resetMultiSigInstanceCalls };
