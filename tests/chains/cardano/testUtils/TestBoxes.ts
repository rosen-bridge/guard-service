...
import {
    ...
    BigNum, GeneralTransactionMetadata, hash_transaction, MetadataMap,
    MultiAsset,
    ScriptHash,
    Transaction,
    TransactionBuilder, TransactionMetadatum,
    TransactionOutput,
...
class TestBoxes {
     ...
     * @param eventId the event trigger id
     */
    static mockMetaDataPaymentTransaction = (outBoxes: TransactionOutput[], eventId: string): PaymentTransaction => {
        const txBuilder = TransactionBuilder.new(CardanoConfigs.txBuilderConfig)
        outBoxes.forEach(box => txBuilder.add_output(box))
     ...
     * @param bankAddress bank address
     */
    static mockValidPaymentTransaction = (event: EventTrigger, bankAddress: string): PaymentTransaction => {
        const lovelacePaymentAmount: BigNum = CardanoConfigs.txMinimumLovelace
        const assetPaymentAmount: BigNum = BigNum.from_str(event.amount)
            .checked_sub(BigNum.from_str(event.bridgeFee))
            .checked_sub(BigNum.from_str(event.networkFee))
     ...
     * @param bankAddress bank address
     */
    static mockPaymentTransactionWithMetadata = (event: EventTrigger, bankAddress: string): PaymentTransaction => {
        const lovelacePaymentAmount: BigNum = CardanoConfigs.txMinimumLovelace
        const assetPaymentAmount: BigNum = BigNum.from_str(event.amount)
            .checked_sub(BigNum.from_str(event.bridgeFee))
            .checked_sub(BigNum.from_str(event.networkFee))
        ...
        return this.mockMetaDataPaymentTransaction([paymentBox, changeBox], event.getId())
    }
