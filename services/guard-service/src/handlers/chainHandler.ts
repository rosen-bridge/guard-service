...
import { HANDSHAKE_CHAIN, HandshakeChain, AbstractHandshakeNetwork } from '@rosen-chains/handshake';
...
class ChainHandler {
  ...
  private generateHandshakeChain = (): HandshakeChain => {
    ...
    switch (GuardsHandshakeConfigs.chainNetworkName) {
      case 'rpc':
        network = new HandshakeRpcNetwork(
          ...
          CallbackLoggerFactory.getInstance().getLogger('HandshakeRpcNetwork'),
          {
    ...
    return new HandshakeChain(
      ...
      TokenHandler.getInstance().getTokenMap(),
      tssSignFunctionWrapper,
      CallbackLoggerFactory.getInstance().getLogger('HandshakeChain')
    );
  };
