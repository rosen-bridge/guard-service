...
const initScanner = () => {
    ...
    const cardanoCommitmentExtractor = new CommitmentExtractor("cardanoCommitment", [CardanoConfigs.cardanoContractConfig().commitmentAddress], CardanoConfigs.cardanoContractConfig().RWTId, ormDataSource)
    const cardanoEventTriggerExtractor = new EventTriggerExtractor("cardanoEventTrigger", ormDataSource, CardanoConfigs.cardanoContractConfig().eventTriggerAddress, CardanoConfigs.cardanoContractConfig().RWTId)
    const ergoCommitmentExtractor = new CommitmentExtractor("ergoCommitment", [ErgoConfigs.ergoContractConfig().commitmentAddress], ErgoConfigs.ergoContractConfig().RWTId, ormDataSource)
    const ergoEventTriggerExtractor = new EventTriggerExtractor("ergoEventTrigger", ormDataSource, ErgoConfigs.ergoContractConfig().eventTriggerAddress, ErgoConfigs.ergoContractConfig().RWTId)
