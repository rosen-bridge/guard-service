    ...
    switch (txConfirmation) {
      case ConfirmationStatus.ConfirmedEnough: {
        ...
        if (tx.type === TransactionType.payment && tx.chain !== ERGO_CHAIN) {
          ...
          logger.info(
            `Tx [${tx.txId}] is confirmed. Event [${
              tx.event!.id
            }] is ready for reward distribution`
          );
        } else if (
    ...
    if (
      ...
      switch (tx.type) {
        case TransactionType.payment:
          ...
          logger.info(
            `Tx [${tx.txId}] is invalid. Event [${
              tx.event!.id
            }] is now waiting for payment. Reason: ${
              invalidationDetails.reason
            }`
          );
          break;
        case TransactionType.reward:
          ...
          logger.info(
            `Tx [${tx.txId}] is invalid. Event [${
              tx.event!.id
            }] is now waiting for reward distribution. Reason: ${
              invalidationDetails.reason
            }`
          );
          break;
