...
class OutputBoxes {
     ...
     */
    static addTokenToBoxBuilder = (box: ErgoBoxCandidateBuilder, tokenId: string, tokenAmount: bigint): void => {
        if (tokenAmount > 0n) box.add_token(TokenId.from_str(tokenId), ErgoUtils.tokenAmountFromBigint(tokenAmount))
    }
    ...
    static createRewardDistributionBoxes = (
        ...
        this.addTokenToBoxBuilder(guardsBridgeFeeBox, paymentTokenId, guardBridgeFeeTokenAmount)
        ...
        this.addTokenToBoxBuilder(guardsNetworkFeeBox, paymentTokenId, guardNetworkTokenAmount)
