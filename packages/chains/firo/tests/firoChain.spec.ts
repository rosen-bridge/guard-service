...
describe('FiroChain', () => {
  ...
  describe('Protected Functions', () => {
    ...
    describe('buildSignedTransaction', () => {
      ...
      it('should build signed PSBT with signatures', async () => {
        ...
        const signatures = ['a'.repeat(128)];

        // run test
        const firoChain = await generateChainObject(network);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const result = (firoChain as any).buildSignedTransaction(
          paymentTx.txBytes,
          signatures,
        );
    ...
    describe('getTransactionsBoxMapping', () => {
      ...
      it('should return empty map for empty transaction list', async () => {
        ...
        const firoChain = await generateChainObject(network);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const result = (firoChain as any).getTransactionsBoxMapping(
          transactions,
          address,
        );
  ...
  describe('submitTransaction', () => {
    ...
    it('should submit transaction successfully', async () => {
      ...
      // mock network submitTransaction
      network.submitTransaction.mockResolvedValue(undefined);
    ...
    /**
     * @target FiroChain.submitTransaction should catch and log network errors
     * @dependencies
     * @scenario
     * - mock PaymentTransaction
     * - mock network submitTransaction to throw error
     * - run test
     * - check that error is caught (not propagated) and logged
     * @expected
     * - it should not throw, matching Doge's error-handling pattern
     */
    it('should catch and log network errors', async () => {
      ...
      // run test — should not throw
      const firoChain = await generateChainObject(network);
      await firoChain.submitTransaction(paymentTx);

      // verify the network method was still called
      expect(network.submitTransaction).toHaveBeenCalled();
    });
  });
  ...
  describe('signTransaction', () => {
    ...
    it('should call signing function for each input', async () => {
      let signCallCount = 0;
      const testSignMediator: EcdsaSignMediator = {
        sign: async () => {
          signCallCount++;
          return {
    ...
    it('should throw error when at least signing of one message is failed', async () => {
      // mock a sign mediator that throws error
      const failingSignMediator: EcdsaSignMediator = {
        sign: async () => {
  ...
  describe('rawTxToPaymentTransaction', () => {
    ...
    it('should construct transaction successfully', async () => {
      ...
      expect(Array.isArray((result as FiroTransaction).inputUtxos)).toBe(true);
    });
