...
class FiroRpcNetwork extends AbstractFiroNetwork {
  ...
  submitTransaction = async (transaction: Psbt): Promise<void> => {
    ...
    console.log(`\x1b[36m[FIRO RPC]\x1b[0m Calling sendrawtransaction RPC...`);
    console.log(`\x1b[36m[FIRO RPC]\x1b[0m Transaction hex length: ${txHex.length} bytes`);
    ...
    try {
      ...
      console.log(`\x1b[32m[FIRO RPC]\x1b[0m ✓ Transaction broadcast! TXID: ${response.data.result}`);
      this.logger?.debug(
   ...
   */
  getAddressAssets = async (address: string): Promise<{ nativeToken: bigint; tokens: Array<{ id: string; value: bigint }> }> => {
    const randomId = this.generateRandomId();
    try {
      // Get list of unspent outputs for the address
      const response = await this.client.post<JsonRpcResult>('', {
